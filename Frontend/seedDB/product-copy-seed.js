...
const Product = require("../models/product-copy");
...
let shirts=[ "L",  "XL",  "XXL",  "3XL",  "S",  "XS",]
let pants=["34","36","38","40","42","44","46"]
let shoes=["7","8","9","10","11","12","13"]
let brands=[
  "Nike",
  "puma",
  "Rebook",
...
const gender = [
  ...
  async function seedProducts(titlesArr, imgsArr, categStr,sizeArr) {
    try {
      const categ = await Category.findOne({ title: categStr });
      let count = await Seller.countDocuments();
      let sr = Math.floor(Math.random()*count);
      const seller = await Seller.find().skip(sr).exec();
       // console.log(seller[0]);
      for (let i = 0; i < titlesArr.length; i++) {
       
        let rad = Math.floor((Math.random()*sizeArr.length)+1);
        let size=[]
        let sum=0;
        for(let j=0; j<rad && j<sizeArr.length; j++){
        ...
        let imgs=[]
        let ir = Math.floor((Math.random()*5)+1);
        for(let a=0; a<imgsArr.length && a<ir; a++){
            imgs.push(imgsArr[a]);
        }
        
        let mrp= faker.random.number({ min: 50, max: 50000 });
        let offer =  faker.random.number({ min: 0, max: 100 });
        let price = mrp*(100-offer)/100;
        let feature={
            brand:brands[Math.floor(Math.random()*(brands.length-1))],
            color:colors[Math.floor(Math.random()*(colors.length-1))].toLowerCase(),
            material:faker.commerce.productMaterial(),
            suitedFor:gender[Math.floor(Math.random()*7)]
        }
        let sellerOb={
        ...
        let prod = new Product({
          ...
          feature:feature,
          sizes:size,
          manufacturer: sellerOb,
          available: sum>0?true:false,
  ...
  await seedProducts(backpacks_titles, backpacks_imgs, "Backpacks", shirts);
  await seedProducts(briefcases_titles, briefcases_imgs, "Briefcases", pants);
  await seedProducts(travel_titles, travel_imgs, "Travel", shirts);
  await seedProducts(miniBags_titles, miniBags_imgs, "Mini Bags", shoes);
  await seedProducts(
  ...
  await seedProducts(purses_titles, purses_imgs, "Purses", shoes);
  await seedProducts(totes_titles, totes_imgs, "Totes", shirts);
